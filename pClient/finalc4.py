...
class MyRob(CRobLinkAngs):
    ...
    def run(self):
        ...
        if os.path.exists("myrob.path"):
            os.remove("myrob.path")
        ...
        while True:
            ...
            if state=='end':
                ...
                for i in self.beacon_mapping:
                    ...
                    self.map[x][y] =  str(i)     
                if os.path.exists("myrob.map"):
                    os.remove("myrob.map")
                with open("myrob.map", 'w') as mapfile:
    ...
    def choose_next(self, state):
        ...
        if self.measures.ground != -1:
            if self.measures.ground not in self.beacon_positions:
                ...
                self.beacon_mapping[self.measures.ground] = self.curr_mapping    
                    #self.beacons += 1   # quantos beacons ele encontrou
        ...
        if self.beacon in self.beacon_positions: ### get id for dictionary 
            ...
            with open("myrob.path", 'a+') as planfile:
                        #print("escrevendo no ficheiro...")
    ...
    def move_one(self):
        ...
        if abs(math.hypot(self.curr_gps[0] - self.curr_x, self.curr_gps[1] - self.curr_y)) <= near and self.measures.irSensor[center] < 2.3: # se ainda nao estiver na proxima celula e se nao tiver uma parede a frente...
            ...
            if self.measures.irSensor[center] >= 2.3: # QUANDO NAO ANDOU 2 CASAS CERTAS MAS ASSUME QUE ESTA NO CENTRO DA CELULA PQ TEM UMA PAREDE...
                #print("Distance: ", abs(math.hypot(self.curr_gps[0] - self.curr_x, self.curr_gps[1] - self.curr_y)))
                self.perform_move(0.0)
                #print("Corrigi o erro")
                #print(" ")
                #print("Predict: ", round(self.curr_x,4), round(self.curr_y,4))
                #print("Real: ", round((self.measures.x-self.inicial_x),4), round((self.measures.y-self.inicial_y),4))
