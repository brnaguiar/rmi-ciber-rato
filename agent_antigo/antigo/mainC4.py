...
class MyRob(CRobLinkAngs):
    ...
    def init_position(self):
        ...
        neighbors, wall_neighbors =  self.check_env() # checka se tem paredes ou passagens e retorna as celulas a volta dele livres.

        self.newtowalk_cells.extend(neighbors)

        self.neighborhood = self.newtowalk_cells 
        self.wall_neighborhood = wall_neighbors       
        ...
        pprint.pprint(self.map_connections)
    ...
    def check_env(self):
        ...
        if self.measures.irSensor[center_id] > 1.4:
            ...
            wall_neighborhood.append(
                [
                    self.curr_cell[0]+center[1],
                    self.curr_cell[1]+center[0]
                ]
            )
        ...
        if self.measures.irSensor[back_id] > 1.4:
            ...
            wall_neighborhood.append(
                [
                    self.curr_cell[0]+back[1],
                    self.curr_cell[1]+back[0]
                ]
            )
        ...
        if self.measures.irSensor[left_id] > 1.4:
            ...
            wall_neighborhood.append(
                [
                    self.curr_cell[0]+left[1],
                    self.curr_cell[1]+left[0]
                ]
            ) 
        ...
        if self.measures.irSensor[right_id] > 1.4:
            ...
            wall_neighborhood.append(
                [
                    self.curr_cell[0]+right[1],
                    self.curr_cell[1]+right[0] 
                ] 
            )     
        ...
        return neighborhood, wall_neighborhood  
    ...
    def update_position(self, diff, cardinal):   
        ...
        self.visited_cells.append(self.curr_cell)  if self.curr_cell not in self.visited_cells else print("current cell already visited")   # adicionar current cell se ela nao estiver ja nas visitadas

        neighbors, wall_neighbors =  self.check_env() 

        self.newtowalk_cells.extend([c for c in neighbors if c not in self.newtowalk_cells])       # meter a vizinhança nas walkable cells se nao estiver  nas walkable cells 
        for c in self.newtowalk_cells: 
            if c in self.visited_cells: self.newtowalk_cells.remove(c)   ##nas visitadas e remover as celulas ja visitadas
        ...
        self.wall_neighborhood.extend([w for w in wall_neighbors if w not in self.wall_neighborhood])  
       ...
       #  print(self.map_connections)
        numpy.savez("connections.npz", connections=numpy.array(self.map_connections) , walls=numpy.array(self.wall_neighborhood)) #,, ,      
    ...
    def choose_next(self, state):
        ...
        if self.tree_path == []:
            ...
            if not self.newtowalk_cells:
                ...
                #k#print("MY CELL: ", self.curr_cell)
                #s#print("NEW TO WALK CELLS: ", self.newtowalk_cells[0])
                #çass#print(self.newtowalk_cells)  
                #ca#print(" ")   
    ...
    def perform_move(self, vel):
        if self.measures.irSensor[left] >= 2.4: 
            self.calc_pos(vel, vel)
            self.driveMotors(vel, vel-0.0025)
                
        elif self.measures.irSensor[right] >= 2.4:
            self.calc_pos(vel, vel)
